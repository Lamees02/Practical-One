...
```{r}
# Calculate count of flights per carrier in January
...
As seen in the table above, the OO carrier only had one flight for the month of January hence a NA standard deviation since standard deviation requires two or more data points in order to be computed.

A standard deviation of zero means that all flights for that carrier have the same distance hence no variation.
...
proportion <- flights |>
  summarise(
    total_flights = n(),
    false_delay = sum(dep_delay > 0 & arr_delay <= 0, na.rm = TRUE)
  ) |>
  mutate(proportion = false_delay / total_flights)
...
```{r}
# Identify routes with more than one airline
...
### Worst and best arrival delays

```{r}
arrival_delays_named <- arrival_delays |>
  left_join(airlines, by = "carrier") |>
  select(origin, dest, name, avg_arr_delay)

arrival_delays_named

```

### Greatest difference between worst and best

```{r}
best_worst <- arrival_delays_named |>
  group_by(origin, dest) |>
  summarise(
...
This difference could be due to weather conditions, number of flights per airline or the timing of flights.
...
#Function for typos
typos <- function(column, valid_values){
...
#Check for typos
typos_genders <- typos(data$gender, valid_genders)
typos_disease_status <- typos(data$disease_status, valid_disease_status)
typos_smoker <- typos(data$smoker, valid_smoker)
typos_exercise <- typos(data$exercise, valid_exercise)

missing_vals
typos_genders
typos_disease_status
typos_smoker
typos_exercise
